...
// Function
function getMistralResponse($prompt)
{
    $api_url = "https://sandipbaruwal.onrender.com/mistral?prompt=" . urlencode($prompt);
    ...
    if (isset($_SERVER["HTTP_EXPERIMENTAL"]) && $_SERVER["HTTP_EXPERIMENTAL"] === "true") {
        if (isset($_SERVER["HTTP_REGEX"])) {
            ...
            if (preg_match($regexPattern, $message, $argument)) {
                ...
                $response = getMistralResponse($argument1);
                $replies = ["replies" => [["message" => $response]]];
            }
        }
    } else {
        $response = getMistralResponse($message);
        $replies = ["replies" => [["message" => $response]]];
    }
