...
// Function
function getBingResponse($query)
{
    $api_url = "https://joshweb.click/bing?prompt=" . urlencode($query) . "&mode=" . urlencode($mode);
    $response = @file_get_contents($api_url);
    return $response ? json_decode($response, true)["bing"] : null;
}
    ...
    if (isset($_SERVER["HTTP_EXPERIMENTAL"]) && $_SERVER["HTTP_EXPERIMENTAL"] === "true") {
        if (isset($_SERVER["HTTP_REGEX"])) {
            ...
            if (preg_match($regexPattern, $message, $argument)) {
                ...
                $response = getBingResponse($argument1);
                $replies = ["replies" => [["message" => $response]]];
            }
        }
    } else {
        $response = getBingResponse($message);
        $replies = ["replies" => [["message" => $response]]];
    }
