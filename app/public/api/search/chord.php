...
// Function
function getSongChord($query)
{
    $api_url = "https://joshweb.click/search/chords?q=" . urlencode($query);
    $response = @file_get_contents($api_url);
    return $response ? json_decode($response, true)["chord"] : "Terjadi Error atau Lagu Tidak Ditemukan";
}
    ...
    if (isset($_SERVER["HTTP_EXPERIMENTAL"]) && $_SERVER["HTTP_EXPERIMENTAL"] === "true") {
        if (isset($_SERVER["HTTP_REGEX"])) {
            ...
            if (preg_match($regexPattern, $message, $argument)) {
                ...
                $response = getSongChord($argument1);
