...
// Function
function getLirikResponse($query)
{
    $api_url = "https://api.maher-zubair.tech/search/lyrics?q=" . urlencode($query);
    ...
    if (isset($_SERVER["HTTP_EXPERIMENTAL"]) && $_SERVER["HTTP_EXPERIMENTAL"] === "true") {
        if (isset($_SERVER["HTTP_REGEX"])) {
            ...
            if (preg_match($regexPattern, $message, $argument)) {
                ...
                $response = getLirikResponse($argument1);
                
                $msg = $apimsg . "\n\n" . "Title: " . $response["title"] . "\n Artist: " . $response["artist"] . "\nCover: " . "_Belum Tersedia_" . "\n\n> *_[Hasil Pencarian Lirik]_* \n\n" . $response["lyrics"];
                
                $replies = ["replies" => [["message" => $msg]]];
            }
        }
    } else {
        $response = getLirikResponse($message);
