...
// Function
function getShortlinkResponse($query)
{
    $api_url = "https://api.maher-zubair.tech/misc/shorturl?url=" . urlencode($query);
    ...
    if (isset($_SERVER["HTTP_EXPERIMENTAL"]) && $_SERVER["HTTP_EXPERIMENTAL"] === "true") {
        if (isset($_SERVER["HTTP_REGEX"])) {
            ...
            if (preg_match($regexPattern, $message, $argument)) {
                ...
                $response = getShortlinkResponse($argument1);
        ...
        $response = getShortlinkResponse($message);
