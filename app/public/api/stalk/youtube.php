...
// Function
function getYoutubeResponse($query)
{
    $api_url = "https://toxicdevilapi.vercel.app/stalk/youtube?channel=" . $query;
    ...
    if (isset($_SERVER["HTTP_EXPERIMENTAL"]) && $_SERVER["HTTP_EXPERIMENTAL"] === "true") {
        if (isset($_SERVER["HTTP_REGEX"])) {
            ...
            if (preg_match($regexPattern, $message, $argument)) {
                ...
                $response = getYoutubeResponse($argument1);
           $msg = $apimsg . "\n\n" . "Title: " . $response["title"] . "\nHandler: " . $response["handler"] . "\nUrl: " . $response["url"] . "\nAbout: " . $response["about_channel"] . "\nVerified: " . $response["verified"] . "\nSubscriber: " . $response["subscribers_text"] . "\nPicture: " . $response["profile_pic"];
                  
             $replies = ["replies" => [["message" => $msg]]];
            }
        }
    } else {
        $response = getYoutubeResponse($message);
